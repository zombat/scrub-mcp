...
Usage:
    python -m dspy_code_hygiene.optimizers.tune --examples-dir ./examples
    python -m dspy_code_hygiene.optimizers.tune --modules docstrings,types
    python -m dspy_code_hygiene.optimizers.tune --strategy mipro  # override all
"""
...
from dspy_code_hygiene.config import OptimizerConfig, PipelineConfig, load_config
...
from dspy_code_hygiene.pipeline import configure_dspy
from dspy_code_hygiene.tools.parser import extract_functions_from_file
...
def load_examples(examples_dir: Path) -> dict[str, list[dict]]:
    """Load training examples from annotated Python files.

    Each .py file in examples_dir should have a companion .json file:
    {
        "docstrings": {"func_name": "expected docstring", ...},
        "types": {"func_name": {"param": "type", "return": "type"}, ...},
        "tests": {"func_name": "expected test code", ...},
        "simplifications": {"func_name": [{"kind": "...", ...}], ...}
    }
    ...
    for py_file in sorted(examples_dir.glob("*.py")):
        ...
        for func in functions:
            ...
            # Comments and refactoring use the same base examples
...
def optimize(
    ...
    for module_name, reg in MODULE_REGISTRY.items():
        ...
        try:
            ...
            from dspy_code_hygiene.optimizers.health import save_fingerprint
...
def _build_lm(model_config: "ModelConfig", label: str = "") -> dspy.LM:
    ...
    """
    from dspy_code_hygiene.config import ModelConfig
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Directory with .py + .json training pairs",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
