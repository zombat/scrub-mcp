...
Usage:
    python -m scrub.optimizers.health
    python -m scrub.optimizers.health --threshold 0.7
    python -m scrub.optimizers.health --module docstrings
"""
...
from scrub.optimizers.tune import (
    MODULE_REGISTRY,
    _build_lm,
    load_examples,
)
...
def print_health_report(health: dict[str, dict[str, Any]]) -> None:
    ...
    if stale_count:
        ...
        logger.info(
            "    python -m scrub.optimizers.tune --modules %s",
            ",".join(stale_names),
        )
    else:
