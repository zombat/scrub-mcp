...
Usage:
    python -m scrub.optimizers.tune --examples-dir ./examples
    python -m scrub.optimizers.tune --modules docstrings,types
    python -m scrub.optimizers.tune --strategy mipro  # override all
"""
...
from scrub.config import OptimizerConfig, PipelineConfig, load_config
...
from scrub.examples import bundled_examples_dir
from scrub.pipeline import configure_dspy
from scrub.tools.parser import extract_functions_from_file
...
def _load_from_pairs(examples_dir: Path) -> dict[str, list[dict]]:
    ...
    for messy_file in sorted(examples_dir.glob("*_messy.py")):
        ...
        for func in messy_funcs:
            ...
            all_examples["comments"].append({**base, "complexity": func.cyclomatic_complexity})
...
def optimize(
    ...
    for module_name, reg in MODULE_REGISTRY.items():
        ...
        try:
            ...
            from scrub.optimizers.health import save_fingerprint
...
def _build_lm(model_config: "ModelConfig", label: str = "") -> dspy.LM:
    ...
    """
    from scrub.config import ModelConfig
...
def main() -> None:
    ...
    if args.build_examples:
        from scrub.optimizers.examples_gen import generate_examples
