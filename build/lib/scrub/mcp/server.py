...
Usage:
    python -m scrub.mcp.server
    # Or in MCP config: {"command": "python", "args": ["-m", "scrub.mcp.server"]}
"""
...
from scrub.config import PipelineConfig, load_config
from scrub.pipeline import run_pipeline, configure_dspy
from scrub.models import HygieneReport
from scrub.modules.hygiene import CommentWriter, DocstringGenerator, TypeAnnotator
from scrub.tools.linter import run_ruff
from scrub.tools.parser import extract_functions
from scrub.tools.complexity import analyze_file_complexity
from scrub.tools.dead_code import find_dead_code
from scrub.tools.imports import analyze_imports_deterministic, fix_imports_ruff
from scrub.tools.security import run_bandit
from scrub.tools.sbom import generate_sbom
from scrub.tools.vulnscan import scan_components
from scrub.modules.coding_tools import (
    BatchTestGenerator,
    ComplexityReducer,
        ...
        # ── Coding tool handlers ──
            ...
            for func in functions:
                if target and func.name != target:
                    continue
                from scrub.tools.complexity import analyze_complexity
            ...
            if arguments.get("infer_missing", True) and analysis.potentially_missing:
                ...
                from scrub.tools.parser import extract_module_info
            ...
            if len(funcs) <= bs and bs > 1:
                ...
                from scrub.tools.utils import batch as batch_fn
                gen = BatchTestGenerator()
                all_code = []
                all_covered = []
                total_tests = 0
